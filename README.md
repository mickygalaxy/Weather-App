...
@ The event listener is added to the search button (searchBtn), so when the button is clicked, it triggers the checkWeather function with the value entered in the search 
  input field (searchBox.value).
